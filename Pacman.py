...
# Ghosts
ghosts = [{'x': random.randint(0, WIDTH), 'y': random.randint(0, HEIGHT), 'speed_x': random.choice([-2, 2]),
           'speed_y': random.choice([-2, 2])} for _ in range(3)]

# Pellets
pellets = [{'x': random.randint(0, WIDTH), 'y': random.randint(0, HEIGHT)} for _ in range(20)]
...
while True:
    ...
    for pellet in pellets:
        if pygame.Rect(pellet['x'] - PELLET_RADIUS, pellet['y'] - PELLET_RADIUS, 2 * PELLET_RADIUS,
                       2 * PELLET_RADIUS).colliderect(
                pygame.Rect(pacman_x - PACMAN_RADIUS, pacman_y - PACMAN_RADIUS, 2 * PACMAN_RADIUS, 2 * PACMAN_RADIUS)):
    ...
    for ghost in ghosts:
        ...
        if pygame.Rect(ghost['x'] - GHOST_RADIUS, ghost['y'] - GHOST_RADIUS, 2 * GHOST_RADIUS,
                       2 * GHOST_RADIUS).colliderect(
                pygame.Rect(pacman_x - PACMAN_RADIUS, pacman_y - PACMAN_RADIUS, 2 * PACMAN_RADIUS, 2 * PACMAN_RADIUS)):
    ...
    pygame.draw.polygon(screen, WHITE, [(round(pacman_x), round(pacman_y)),
                                        (round(pacman_x + PACMAN_RADIUS), round(pacman_y - PACMAN_RADIUS)),
                                        (round(pacman_x + PACMAN_RADIUS), round(pacman_y + PACMAN_RADIUS))])

    # Draw Ghosts
    for ghost in ghosts:
        pygame.draw.circle(screen, RED, (round(ghost['x']), round(ghost['y'])), GHOST_RADIUS)

    # Draw Pellets
    for pellet in pellets:
        pygame.draw.circle(screen, WHITE, (round(pellet['x']), round(pellet['y'])), PELLET_RADIUS)
